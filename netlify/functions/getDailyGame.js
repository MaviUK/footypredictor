...
async function checked(step, query) {
  ...
  if (result.error) {
    const parts = [
      `${step}: ${result.error.message || 'Supabase error'}`,
...
async function buildLiveSnapshots(supabase, fixture) {
  const seasonFixturesResult = await checked('season fixtures lookup', supabase
    .from('fixtures')
...
async function assignTierPlace(supabase, country, competition) {
  const profilesResult = await checked('tier allocation lookup', supabase
    .from('user_profiles')
    .select('*')
    .eq('country', country)
...
async function ensureDailyGameFixtures(supabase, dailyGame) {
  const existingRounds = await checked('daily game fixtures count', supabase
  ...
  if ((existingRounds.data || []).length > 0) {
    await checked('daily game fixtures reset', supabase
      .from('daily_game_fixtures')
      .delete()
      .eq('daily_game_id', dailyGame.id))
  }

  const fixturesResult = await checked('fixtures lookup', supabase
    .from('fixtures')
    .select('*')
  ...
  await checked('daily game fixtures insert', supabase
    .from('daily_game_fixtures')
    .insert(gameFixtures))
}
...
async function ensureDailyGame(supabase, gameDate, league) {
  ...
  const seasonsResult = await checked('season lookup', supabase
    .from('seasons')
    .select('*')
    .eq('country', league.country)
    .eq('competition', league.competition)
  ...
  if (!seasonsResult.data?.length) {
    throw new Error(`No complete seasons imported for ${league.country} - ${league.leagueName}`)
  }
...
async function reopenTodaysGameIfAutoClosed(supabase, dailyGame, roundIds, gameDate) {
  ...
  await checked('auto prediction cleanup', supabase
    .from('predictions')
    .delete()
    .eq('is_auto', true)
    .in('daily_game_fixture_id', roundIds))

  if (dailyGame.status !== 'closed') return dailyGame

  await checked('daily results cleanup', supabase
    .from('daily_results')
    .delete()
    .eq('daily_game_id', dailyGame.id))

  const reopenResult = await checked('daily game reopen', supabase
    .from('daily_games')
...
export async function handler(event) {
  try {
    ...
    const profileResult = await checked('profile lookup', supabase
      .from('user_profiles')
      .select('*')
      .eq('user_id', user.id)
      .maybeSingle())
    ...
    const savedProfileResult = await checked('profile upsert', supabase
      .from('user_profiles')
      .upsert({
        user_id: user.id,
    ...
    if (!roundsResult.data?.length) throw new Error('Today\'s game has no fixtures')
    ...
    const predictionsResult = await checked('user predictions lookup', supabase
      .from('predictions')
      .select('*')
      .eq('user_id', user.id)
      .eq('is_auto', false)
    ...
    const resultHistory = userRounds
      .map((round) => {
        ...
        return { roundNumber: round.userRoundNumber, ...predictionOutcome(prediction) }
      })
      .filter(Boolean)

    const rosterResult = await checked('tier roster lookup', supabase
      .from('user_profiles')
      .select('*')
      .eq('country', league.country)
      .eq('competition', league.competition)
    ...
    const leaderboardResult = rosterIds.length
      ? await checked('tier predictions lookup', supabase
        .from('predictions')
    ...
    if (!completed && nextRound) {
      ...
      currentRound = {
        ...
        home: { name: nextRound.fixtures.home_team, snapshot: liveSnapshots.home },
        away: { name: nextRound.fixtures.away_team, snapshot: liveSnapshots.away },
      }
    }
    ...
    return json(200, {
      ...
      season: {
        ...
        displayName: dailyGame.seasons?.display_name || league.leagueName,
      },
    ...
    return json(500, {
      error: error.message || 'Unknown getDailyGame error',
      name: error.name,
